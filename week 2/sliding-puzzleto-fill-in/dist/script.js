...
const Game = () => {
  ...
  function moveBlankLeft(arr, element) {
  ...
  function moveBlankRight(arr, element) {
    let idx = arr.indexOf(element);
    if (idx < arr.length - 1) {
      arr[idx] = arr[idx + 1];
      arr[idx + 1] = element;
  ...
  function moveBlankUp(arr, e) {
    let pos,idx = arr.findIndex(n => [n.includes(e), pos = n.indexOf(e)][0]);
    if (idx > 0) {
      arr[idx].splice(pos, 1, arr[idx - 1][pos]);
      arr[idx - 1].splice(pos, 1, e);
    }
    checkWin(gridWithBlank, arr);
    return arr;
  }
  ...
  const shuffleGrid = () => {
    ...
    let currentIndex = tempFlatGrid.length,randomIndex;
    ...
    while (currentIndex != 0) {
      ...
      [tempFlatGrid[currentIndex], tempFlatGrid[randomIndex]] = [tempFlatGrid[randomIndex], tempFlatGrid[currentIndex]];
    }
